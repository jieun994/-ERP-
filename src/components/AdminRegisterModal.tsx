            ...
            <div className="p-6">
              {isSuccess ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        required
                        type="text" 
                        className="w-full pl-9 pr-4 py-2.5 bg-white border border-gray-300 rounded-lg text-body outline-none focus:border-primary transition-all"
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-body-sm font-bold text-gray-700">이메일(아이디) (임시 비밀번호 발송)</label>
                    <div className="relative">
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-body-sm font-bold text-gray-700">권한 등급 선택</label>
                    <div className="grid grid-cols-1 gap-2">
                       {[
                         ...
                         <label key={role.id} className={`flex items-center gap-3 p-3.5 border rounded-lg cursor-pointer transition-all ${formData.role === role.id ? 'border-primary bg-primary/5' : 'border-gray-200 hover:bg-gray-50'}`}>
                           <input 
                             type="radio" 
                             name="role" 
                             className="w-4 h-4 text-primary focus:ring-0 accent-[#008d75]" 
                             checked={formData.role === role.id}
                             onChange={() => setFormData({...formData, role: role.id as any})}
                           />
                           <div className="flex flex-col">
                             <span className={`text-body-sm font-bold ${formData.role === role.id ? 'text-primary' : 'text-gray-700'}`}>{role.label}</span>
                           </div>
                         </label>
                       ))}
                    </div>
                       ...
                       <div className="flex items-center gap-6">
                         <label className="flex items-center gap-2 cursor-pointer">
                           <input 
                             type="radio" 
                             name="isUsed"
                             className="w-4 h-4 text-primary focus:ring-[#008d75]" 

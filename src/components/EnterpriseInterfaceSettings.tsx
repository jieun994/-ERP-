...
function makeDefaultInterfaceConfigs(): Record<string, InterfaceConfig> {
  ...
  INTERFACE_DEFS.forEach(def => {
    ...
    configs[def.id] = { enabled: false, paramEndpoints };
  });
  return configs;
}
      ...
      <div className="flex gap-6" style={{ minHeight: 560 }}>
        ...
        <div className="w-[200px] flex-shrink-0 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="overflow-y-auto">
            {enterprises.map(ent => {
              ...
              const nameCls = isActive ? 'text-primary' : 'text-gray-800';
              ...
              return (
                ...
                >
                  <div className="min-w-0">
                    <div className={'text-body font-semibold truncate ' + nameCls}>{ent.name}</div>
                    <div className="text-body-sm text-gray-400 mt-0.5">
                      {status === 'done' ? '입력 완료' : '미입력'}
                    </div>
                  </div>
                  <span className={'text-caption px-2 py-0.5 rounded-full border whitespace-nowrap flex-shrink-0 ' + badgeCls}>
                    {status === 'done' ? '완료' : '대기'}
                  </span>
                </button>
              );
        ...
        <div className="flex-1 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between gap-3">
            <div className="text-body-lg font-bold text-gray-900">{selectedEnt?.name}</div>
            <button
              onClick={handleReset}
